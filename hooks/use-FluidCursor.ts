...
const initFluidCursor = () => {
  ...
  setTimeout(() => {
    ...
    if (!canvas) {
      console.error('Fluid canvas element not found');
      return;
    }
  ...
  // Use a separate canvas element for the fluid effect
  ...
  if (fluidCanvas) {
    fluidCanvas.addEventListener('mousedown', (e) => {
      ...
      clickSplat(pointer);
      // Prevent this event from interfering with other click events
      e.stopPropagation();
    }, { passive: false });
  }
  ...
  update();
  }, 100); // Wait 100ms for the canvas to be available
};

// Create a React hook that follows the rules of hooks
const useFluidCursor = () => {
  return { initFluidCursor };
};
