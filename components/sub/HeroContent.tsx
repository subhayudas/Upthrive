...
const HeroContent = () => {
  ...
  const shadowColor = resolvedTheme === "dark" ? "white" : "white";
     ...
     className="flex flex-col items-center justify-center text-center px-4 py-10 sm:py-16 lg:py-24 bg-tresprent min-h-screen">
      ...
      <motion.h1 variants={slideInFromLeft(0.6)} className="mx-auto max-w-4xl font-display text-5xl font-medium tracking-tight text-white sm:text-7xl">
        ...
        <span className="relative whitespace-nowrap text-orange-600">
          ...
          <LineShadowText className="relative inline-block py-0 text-5xl sm:text-7xl font-medium text-white" shadowColor={shadowColor} as="span">ambitious brands</LineShadowText>
        </span>
      </motion.h1>
      <motion.div 
        variants={slideInFromRight(0.8)} 
        className="mx-auto mt-6 max-w-xl text-base sm:text-lg lg:text-xl text-white leading-7 overflow-hidden"
      >
        {["We craft digital experiences that captivate audiences,", "drive conversions, and elevate your brand story.", "Partner with us to transform your vision into digital reality."].map((text, index) => (
          <motion.p 
            key={index}
            ...
            className="mb-2 relative group overflow-hidden"
          >
            <span className="relative inline-block animate-shiny-text bg-gradient-to-r from-white via-orange-400 via-50% to-white bg-clip-text text-transparent bg-[length:400%_100%]">
              {text}
            </span>
          </motion.p>
        ))}
